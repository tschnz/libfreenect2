...
namespace libfreenect2
{
  ...
  public:
    ...
    OpenCLKdeDepthPacketProcessorImpl(const int deviceId = -1)
        : deviceInitialized(false), programBuilt(false), programInitialized(false), runtimeOk(true)
    {
...
#pragma GCC diagnostic pop
  ...
  OpenCLKdeDepthPacketProcessor::OpenCLKdeDepthPacketProcessor(const int deviceId) : impl_(new OpenCLKdeDepthPacketProcessorImpl(deviceId))
  {
  }
  ...
  void OpenCLKdeDepthPacketProcessor::setConfiguration(const libfreenect2::DepthPacketProcessor::Config &config)
  {
    ...
    if (impl_->config.MaxDepth != config.MaxDepth || impl_->config.MinDepth != config.MinDepth)
    {
      // OpenCL program needs to be rebuilt, then reinitialized
      impl_->programBuilt = false;
      impl_->programInitialized = false;
    }
    else if (impl_->config.EnableBilateralFilter != config.EnableBilateralFilter || impl_->config.EnableEdgeAwareFilter != config.EnableEdgeAwareFilter)
    {
