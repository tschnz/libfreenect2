...
#define IMPORT(func)                                                        \
  func = reinterpret_cast<TYPEOF(&nv_headers::func)>(dlsym(handle, #func)); \
  if ((err = dlerror()))                                                    \
  {                                                                         \
    LOG_ERROR << "dlsym: " << err;                                          \
    dlclose(handle);                                                        \
...
namespace libfreenect2
{
  ...
  public:
    ...
    TegraImageAllocator(void *owner, struct jpeg_error_mgr *jerr, libjpeg_handle &libjpeg) : owner(owner), jerr(jerr), libjpeg(libjpeg) {}
  ...
  {
  public:
    TegraFrame(size_t width, size_t height, size_t bpp, TegraImage *ti) : Frame(width, height, bpp, (unsigned char *)-1)
    {
      data = NULL;
  ...
  public:
    ...
    TegraJpegRgbPacketProcessorImpl() : libjpeg(),
                                        good(true),
                                        frame(NULL),
  ...
  TegraJpegRgbPacketProcessor::TegraJpegRgbPacketProcessor() : impl_(new TegraJpegRgbPacketProcessorImpl())
  {
  }
