...
namespace libfreenect2
{
  ...
  public:
    ...
    VaapiImageAllocator(VADisplay display, unsigned short width, unsigned short height, const VAImageFormat &format) : display(display), width(width), height(height), format(format)
    {
    }
  ...
  {
  public:
    VaapiFrame(VaapiImage *vi) : Frame(vi->image.width, vi->image.height, vi->image.format.bits_per_pixel / 8, (unsigned char *)-1)
    {
      data = NULL;
  ...
  public:
    ...
    VaapiRgbPacketProcessorImpl() : frame(NULL),
  ...
  VaapiRgbPacketProcessor::VaapiRgbPacketProcessor() : impl_(new VaapiRgbPacketProcessorImpl())
  {
  }
